...
#
# Build and run the JMH benchmarks for the collector module.
#
# The benchmarks are packaged as a self-contained executable jar by the
# collector-benchmarks module. This script builds that jar and then runs JMH
# against it.
#
# Usage: ./scripts/collector-benchmarks.sh [options] [-- <jmh args>]
#
# Options:
#   --no-build   Skip building collector-benchmarks/target/benchmarks.jar
...
#
# Examples:
#   ./scripts/collector-benchmarks.sh --quick
#   ./scripts/collector-benchmarks.sh --quick JmxCollectorBenchmark
#   ./scripts/collector-benchmarks.sh --no-build -- -f 1 -wi 3 -i 3 ".*Cache.*"
#
...
readonly MODULE_DIR="${ROOT_DIR}/collector-benchmarks"
...
build() {
    log_info "Building collector-benchmarks (tests skipped)"
    (
        cd "${ROOT_DIR}"
        ./mvnw -B -pl collector-benchmarks -am -DskipTests package
    )
}
