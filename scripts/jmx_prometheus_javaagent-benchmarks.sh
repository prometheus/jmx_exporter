...
#
# Build and run the JMH benchmarks for the jmx_prometheus_javaagent module.
#
# The benchmarks are packaged as a self-contained executable jar by the
# jmx_prometheus_javaagent-benchmarks module. This script builds that jar and
# then runs JMH against it.
#
# Usage: ./scripts/jmx_prometheus_javaagent-benchmarks.sh [options] [-- <jmh args>]
#
# Options:
#   --no-build   Skip building jmx_prometheus_javaagent-benchmarks/target/benchmarks.jar
...
#
# Examples:
#   ./scripts/jmx_prometheus_javaagent-benchmarks.sh --quick
#   ./scripts/jmx_prometheus_javaagent-benchmarks.sh --quick '.*ArgumentsBenchmark.*'
#   ./scripts/jmx_prometheus_javaagent-benchmarks.sh --no-build -- -f 1 -wi 3 -i 3 ".*AgentStartupBenchmark.*"
#
...
readonly MODULE_DIR="${ROOT_DIR}/jmx_prometheus_javaagent-benchmarks"
...
build() {
    log_info "Building jmx_prometheus_javaagent-benchmarks (tests skipped)"
    (
        cd "${ROOT_DIR}"
        ./mvnw -B -pl jmx_prometheus_javaagent-benchmarks -am -DskipTests package
    )
}
