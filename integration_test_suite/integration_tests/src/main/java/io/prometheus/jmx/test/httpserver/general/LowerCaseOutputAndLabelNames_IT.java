...
public class LowerCaseOutputAndLabelNames_IT {
    ...
    public void beforeAll() throws Exception {
        ...
        // HTTP client
        httpClient = new HttpClient("http://localhost:" + exporterContainer.getMappedPort(8888));
    }
    ...
    private static void assertMetrics(Response response) throws IOException {
        ...
        Optional <Metric> optional =
                metricList
                        .stream()
                        .filter(m ->
                                m.getName().equals("java_lang_Memory_NonHeapMemoryUsage_committed".toLowerCase()))
                        .findFirst();
        ...
        optional =
                metricList
                        .stream()
                        .filter(m ->
                                m.getName().equals("io_prometheus_jmx_tabularData_Server_1_Disk_Usage_Table_size".toLowerCase())
