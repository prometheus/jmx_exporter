...
public class AuthenticatorPluginWithThreadsTest {
    ...
    public static Action factory() throws Throwable {
        return Each.parallel(
                        AuthenticatorPluginWithThreadsTest.class.getName(),
                        JmxExporterTestEnvironment.createTestEnvironments(AuthenticatorPluginWithThreadsTest.class)
                                .stream()
                                .filter(environment -> environment.getJmxExporterMode() == JmxExporterMode.JavaAgent)
                                .collect(Collectors.toList()),
                        environment -> instance(
                                        environment.name(), () -> new AuthenticatorPluginWithThreadsTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        AuthenticatorPluginWithThreadsTest.class,
                                                        AuthenticatorPluginWithThreadsTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                AuthenticatorPluginWithThreadsTest.class,
                                                                AuthenticatorPluginWithThreadsTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                AuthenticatorPluginWithThreadsTest.class,
                                                                AuthenticatorPluginWithThreadsTest
                                                                        ::testDefaultTextMetrics)))
                                                .child(step(
                                        ...
                                        .after(step(
                                                ...
                                                withInstance(
                                                        AuthenticatorPluginWithThreadsTest.class,
                                                        AuthenticatorPluginWithThreadsTest::tearDown)))))
                .build();
    }

    private AuthenticatorPluginWithThreadsTest(JmxExporterTestEnvironment environment) {
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(AuthenticatorPluginWithThreadsTest.class, javaDockerImage, mode, metrics);
    }
}
