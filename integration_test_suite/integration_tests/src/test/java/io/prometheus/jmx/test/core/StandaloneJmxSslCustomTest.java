...
public class StandaloneJmxSslCustomTest {

    private final JmxExporterTestEnvironment environment;
    private Network network;

    private StandaloneJmxSslCustomTest(JmxExporterTestEnvironment environment) {
    ...
    public static Action factory() throws Throwable {
        var environments = JmxExporterTestEnvironment.createTestEnvironments(StandaloneJmxSslCustomTest.class).stream()
                .filter(environment -> environment.getJmxExporterMode() == JmxExporterMode.Standalone)
                .filter(environment -> !environment.getJavaDockerImage().contains("graalvm/jdk:java8"))
                .filter(environment -> !environment.getJavaDockerImage().contains("ibmjava"))
                .collect(Collectors.toList());

        return Each.parallel(
                        StandaloneJmxSslCustomTest.class.getName(),
                        environments,
                        environment -> instance(environment.name(), () -> new StandaloneJmxSslCustomTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        StandaloneJmxSslCustomTest.class,
                                                        StandaloneJmxSslCustomTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                StandaloneJmxSslCustomTest.class,
                                                                StandaloneJmxSslCustomTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                StandaloneJmxSslCustomTest.class,
                                                                StandaloneJmxSslCustomTest::testDefaultTextMetrics)))
                                                .child(step(
                                                        "testOpenMetricsTextMetrics()",
                                                        withInstance(
                                                                StandaloneJmxSslCustomTest.class,
                                                                StandaloneJmxSslCustomTest
                                                                        ::testOpenMetricsTextMetrics)))
                                                .child(step(
                                                        "testPrometheusTextMetrics()",
                                                        withInstance(
                                                                StandaloneJmxSslCustomTest.class,
                                                                StandaloneJmxSslCustomTest::testPrometheusTextMetrics)))
                                                .child(step(
                                        ...
                                        .after(step(
                                                ...
                                                withInstance(
                                                        StandaloneJmxSslCustomTest.class,
                                                        StandaloneJmxSslCustomTest::tearDown)))))
                .build();
    }
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(StandaloneJmxSslCustomTest.class, javaDockerImage, mode, metrics);
    }
}
