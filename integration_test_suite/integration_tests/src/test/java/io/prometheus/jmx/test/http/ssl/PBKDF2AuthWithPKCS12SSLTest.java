...
public class PBKDF2AuthWithPKCS12SSLTest {
    ...
    private PBKDF2AuthWithPKCS12SSLTest(JmxExporterTestEnvironment environment) {
    ...
    public static Action factory() throws Throwable {
        var environments = JmxExporterTestEnvironment.createTestEnvironments(PBKDF2AuthWithPKCS12SSLTest.class).stream()
                .filter(new PKCS12KeyStoreExporterTestEnvironmentFilter())
                .map(environment -> environment.setBaseUrl(BASE_URL))
                .toList();

        return Each.parallel(
                        PBKDF2AuthWithPKCS12SSLTest.class.getName(),
                        environments,
                        environment -> instance(environment.name(), () -> new PBKDF2AuthWithPKCS12SSLTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        PBKDF2AuthWithPKCS12SSLTest.class,
                                                        PBKDF2AuthWithPKCS12SSLTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                PBKDF2AuthWithPKCS12SSLTest.class,
                                                                PBKDF2AuthWithPKCS12SSLTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                PBKDF2AuthWithPKCS12SSLTest.class,
                                                                PBKDF2AuthWithPKCS12SSLTest::testDefaultTextMetrics)))
                                                .child(step(
                                                        "testOpenMetricsTextMetrics()",
                                                        withInstance(
                                                                PBKDF2AuthWithPKCS12SSLTest.class,
                                                                PBKDF2AuthWithPKCS12SSLTest
                                                                        ::testOpenMetricsTextMetrics)))
                                                .child(step(
                                        ...
                                        .after(step(
                                                ...
                                                withInstance(
                                                        PBKDF2AuthWithPKCS12SSLTest.class,
                                                        PBKDF2AuthWithPKCS12SSLTest::tearDown)))))
                .build();
    }
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(PBKDF2AuthWithPKCS12SSLTest.class, javaDockerImage, mode, metrics);
    }
}
