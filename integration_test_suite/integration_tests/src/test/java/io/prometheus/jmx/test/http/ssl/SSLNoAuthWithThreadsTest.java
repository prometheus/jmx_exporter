...
public class SSLNoAuthWithThreadsTest {
    ...
    public static Action factory() throws Throwable {
        var environments = JmxExporterTestEnvironment.createTestEnvironments(SSLNoAuthWithThreadsTest.class).stream()
        ...
        return Each.parallel(
                        SSLNoAuthWithThreadsTest.class.getName(),
                        environments,
                        environment -> instance(environment.name(), () -> new SSLNoAuthWithThreadsTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        SSLNoAuthWithThreadsTest.class,
                                                        SSLNoAuthWithThreadsTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                SSLNoAuthWithThreadsTest.class,
                                                                SSLNoAuthWithThreadsTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                SSLNoAuthWithThreadsTest.class,
                                                                SSLNoAuthWithThreadsTest::testDefaultTextMetrics)))
                                                .child(step(
                                                        "testOpenMetricsTextMetrics()",
                                                        withInstance(
                                                                SSLNoAuthWithThreadsTest.class,
                                                                SSLNoAuthWithThreadsTest::testOpenMetricsTextMetrics)))
                                                .child(step(
                                                        "testPrometheusTextMetrics()",
                                                        withInstance(
                                                                SSLNoAuthWithThreadsTest.class,
                                                                SSLNoAuthWithThreadsTest::testPrometheusTextMetrics)))
                                                .child(step(
                                                        "testPrometheusProtobufMetrics()",
                                                        withInstance(
                                                                SSLNoAuthWithThreadsTest.class,
                                                                SSLNoAuthWithThreadsTest
                                                                        ::testPrometheusProtobufMetrics))))
                                        .after(step(
                                                "tearDown()",
                                                withInstance(
                                                        SSLNoAuthWithThreadsTest.class,
                                                        SSLNoAuthWithThreadsTest::tearDown)))))
                .build();
    }

    private SSLNoAuthWithThreadsTest(JmxExporterTestEnvironment environment) {
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(SSLNoAuthWithThreadsTest.class, javaDockerImage, mode, metrics);
    }
}
