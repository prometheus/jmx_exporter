...
public class StandaloneJmxAuthTest {

    private final JmxExporterTestEnvironment environment;
    private Network network;

    private StandaloneJmxAuthTest(JmxExporterTestEnvironment environment) {
    ...
    public static Action factory() throws Throwable {
        var environments = JmxExporterTestEnvironment.createTestEnvironments(StandaloneJmxAuthTest.class).stream()
                .filter(environment -> environment.getJmxExporterMode() == JmxExporterMode.Standalone)
                .collect(Collectors.toList());

        return Each.parallel(
                        StandaloneJmxAuthTest.class.getName(),
                        environments,
                        environment -> instance(environment.name(), () -> new StandaloneJmxAuthTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        StandaloneJmxAuthTest.class, StandaloneJmxAuthTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                StandaloneJmxAuthTest.class,
                                                                StandaloneJmxAuthTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                StandaloneJmxAuthTest.class,
                                                                StandaloneJmxAuthTest::testDefaultTextMetrics)))
                                                .child(step(
                                                        "testOpenMetricsTextMetrics()",
                                                        withInstance(
                                                                StandaloneJmxAuthTest.class,
                                                                StandaloneJmxAuthTest::testOpenMetricsTextMetrics)))
                                                .child(step(
                                                        "testPrometheusTextMetrics()",
                                                        withInstance(
                                                                StandaloneJmxAuthTest.class,
                                                                StandaloneJmxAuthTest::testPrometheusTextMetrics)))
                                                .child(step(
                                                        "testPrometheusProtobufMetrics()",
                                                        withInstance(
                                                                StandaloneJmxAuthTest.class,
                                                                StandaloneJmxAuthTest::testPrometheusProtobufMetrics))))
                                        .after(step(
                                                "tearDown()",
                                                withInstance(
                                                        StandaloneJmxAuthTest.class,
                                                        StandaloneJmxAuthTest::tearDown)))))
                .build();
    }
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(StandaloneJmxAuthTest.class, javaDockerImage, mode, metrics);
    }
}
