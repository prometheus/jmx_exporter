...
public class ExcludeJvmWithLowerCaseOutputTest {
    ...
    public static Action factory() throws Throwable {
        var environments =
                JmxExporterTestEnvironment.createTestEnvironments(ExcludeJvmWithLowerCaseOutputTest.class).stream()
                        .filter(exporterTestEnvironment ->
                                exporterTestEnvironment.getJmxExporterMode() == JmxExporterMode.JavaAgent)
                        .collect(Collectors.toList());

        return Each.parallel(
                        ExcludeJvmWithLowerCaseOutputTest.class.getName(),
                        environments,
                        environment -> instance(
                                        environment.name(), () -> new ExcludeJvmWithLowerCaseOutputTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        ExcludeJvmWithLowerCaseOutputTest.class,
                                                        ExcludeJvmWithLowerCaseOutputTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                ExcludeJvmWithLowerCaseOutputTest.class,
                                                                ExcludeJvmWithLowerCaseOutputTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                ExcludeJvmWithLowerCaseOutputTest.class,
                                                                ExcludeJvmWithLowerCaseOutputTest
                                                                        ::testDefaultTextMetrics)))
                                                .child(step(
                                        ...
                                        .after(step(
                                                ...
                                                withInstance(
                                                        ExcludeJvmWithLowerCaseOutputTest.class,
                                                        ExcludeJvmWithLowerCaseOutputTest::tearDown)))))
                .build();
    }

    private ExcludeJvmWithLowerCaseOutputTest(JmxExporterTestEnvironment environment) {
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(ExcludeJvmWithLowerCaseOutputTest.class, javaDockerImage, mode, metrics);
    }
}
