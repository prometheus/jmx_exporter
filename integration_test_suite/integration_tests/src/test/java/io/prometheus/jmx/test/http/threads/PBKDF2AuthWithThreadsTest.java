...
public class PBKDF2AuthWithThreadsTest {
    ...
    public static Action factory() throws Throwable {
        return Each.parallel(
                        PBKDF2AuthWithThreadsTest.class.getName(),
                        JmxExporterTestEnvironment.createTestEnvironments(PBKDF2AuthWithThreadsTest.class),
                        environment -> instance(environment.name(), () -> new PBKDF2AuthWithThreadsTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        PBKDF2AuthWithThreadsTest.class,
                                                        PBKDF2AuthWithThreadsTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                PBKDF2AuthWithThreadsTest.class,
                                                                PBKDF2AuthWithThreadsTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                PBKDF2AuthWithThreadsTest.class,
                                                                PBKDF2AuthWithThreadsTest::testDefaultTextMetrics)))
                                                .child(step(
                                                        "testOpenMetricsTextMetrics()",
                                                        withInstance(
                                                                PBKDF2AuthWithThreadsTest.class,
                                                                PBKDF2AuthWithThreadsTest::testOpenMetricsTextMetrics)))
                                                .child(step(
                                                        "testPrometheusTextMetrics()",
                                                        withInstance(
                                                                PBKDF2AuthWithThreadsTest.class,
                                                                PBKDF2AuthWithThreadsTest::testPrometheusTextMetrics)))
                                                .child(step(
                                                        "testPrometheusProtobufMetrics()",
                                                        withInstance(
                                                                PBKDF2AuthWithThreadsTest.class,
                                                                PBKDF2AuthWithThreadsTest
                                                                        ::testPrometheusProtobufMetrics))))
                                        .after(step(
                                                "tearDown()",
                                                withInstance(
                                                        PBKDF2AuthWithThreadsTest.class,
                                                        PBKDF2AuthWithThreadsTest::tearDown)))))
                .build();
    }

    private PBKDF2AuthWithThreadsTest(JmxExporterTestEnvironment environment) {
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(PBKDF2AuthWithThreadsTest.class, javaDockerImage, mode, metrics);
    }
}
