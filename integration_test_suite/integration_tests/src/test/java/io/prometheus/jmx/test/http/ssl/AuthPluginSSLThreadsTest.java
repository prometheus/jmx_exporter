...
public class AuthPluginSSLThreadsTest {
    ...
    public static Action factory() throws Throwable {
        var environments = JmxExporterTestEnvironment.createTestEnvironments(AuthPluginSSLThreadsTest.class).stream()
        ...
        return Each.parallel(
                        AuthPluginSSLThreadsTest.class.getName(),
                        environments,
                        environment -> instance(environment.name(), () -> new AuthPluginSSLThreadsTest(environment))
                                .body(scope("scenario")
                                        .before(step(
                                                "setUp()",
                                                withInstance(
                                                        AuthPluginSSLThreadsTest.class,
                                                        AuthPluginSSLThreadsTest::setUp)))
                                        .body(sequential("tests")
                                                .child(step(
                                                        "testHealthy()",
                                                        withInstance(
                                                                AuthPluginSSLThreadsTest.class,
                                                                AuthPluginSSLThreadsTest::testHealthy)))
                                                .child(step(
                                                        "testDefaultTextMetrics()",
                                                        withInstance(
                                                                AuthPluginSSLThreadsTest.class,
                                                                AuthPluginSSLThreadsTest::testDefaultTextMetrics)))
                                                .child(step(
                                                        "testOpenMetricsTextMetrics()",
                                                        withInstance(
                                                                AuthPluginSSLThreadsTest.class,
                                                                AuthPluginSSLThreadsTest::testOpenMetricsTextMetrics)))
                                                .child(step(
                                                        "testPrometheusTextMetrics()",
                                                        withInstance(
                                                                AuthPluginSSLThreadsTest.class,
                                                                AuthPluginSSLThreadsTest::testPrometheusTextMetrics)))
                                                .child(step(
                                                        "testPrometheusProtobufMetrics()",
                                                        withInstance(
                                                                AuthPluginSSLThreadsTest.class,
                                                                AuthPluginSSLThreadsTest
                                                                        ::testPrometheusProtobufMetrics))))
                                        .after(step(
                                                "tearDown()",
                                                withInstance(
                                                        AuthPluginSSLThreadsTest.class,
                                                        AuthPluginSSLThreadsTest::tearDown)))))
                .build();
    }

    private AuthPluginSSLThreadsTest(JmxExporterTestEnvironment environment) {
    ...
    private void assertMetricsResponse(HttpResponse httpResponse, MetricsContentType metricsContentType) {
        ...
        assertMetrics(AuthPluginSSLThreadsTest.class, javaDockerImage, mode, metrics);
    }
}
