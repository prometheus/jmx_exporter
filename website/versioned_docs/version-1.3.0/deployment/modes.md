...
JMX Exporter 1.3.0 can expose metrics over HTTP, export through OpenTelemetry, or run both exporters from one configuration.
...
- Java agent: `-javaagent:jmx_prometheus_javaagent-1.3.0.jar=9404:exporter.yaml`
- Standalone: `java -jar jmx_prometheus_standalone-1.3.0.jar 9404 exporter.yaml`
...
See [Isolator Java Agent](isolator-java-agent) when multiple isolated exporters must run in one JVM.
