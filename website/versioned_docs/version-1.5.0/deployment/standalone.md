...
Download `jmx_prometheus_standalone-1.5.0.jar` from the [1.5.0 release](https://github.com/prometheus/jmx_exporter/releases/download/1.5.0/jmx_prometheus_standalone-1.5.0.jar).
...
```bash
java -jar jmx_prometheus_standalone-1.5.0.jar 9404 exporter.yaml
```
...
Use top-level `username` and `password` for remote JMX credentials. Supports variable resolution in 1.5.0.
...
```bash
java -jar jmx_prometheus_standalone-1.5.0.jar exporter.yaml
```
