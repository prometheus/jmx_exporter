...
JMX Exporter 1.4.0 uses an exporter YAML file. The same file can contain collector configuration, HTTP server configuration, and OpenTelemetry configuration.
...
| JVM metric exclusion | `excludeJvmMetrics` | [Object Names](object-names) |
...
Only document and use keys that are supported by the 1.4.0 implementation. The [configuration reference](../reference/configuration) lists the source-backed keys.
