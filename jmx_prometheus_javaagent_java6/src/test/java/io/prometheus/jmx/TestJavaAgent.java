...
public class TestJavaAgent {
    ...
    public void testAgentStringParsing() {
        ...
        Assert.assertFalse(config.disableDefaultExports);
        ...
        config = JavaAgent.parseConfig("disableDefaultExports:127.0.0.1:8080:C:\\Windows\\Path\\config.yaml", DEFAULT_HOST);
        Assert.assertEquals("127.0.0.1", config.host);
        Assert.assertEquals("C:\\Windows\\Path\\config.yaml", config.file);
        Assert.assertEquals(8080, config.port);
        Assert.assertTrue(config.disableDefaultExports);

        config = JavaAgent.parseConfig("disableDefaultExports:8080:C:\\Windows\\Path\\config.yaml", DEFAULT_HOST);
        Assert.assertEquals(DEFAULT_HOST, config.host);
        Assert.assertEquals("C:\\Windows\\Path\\config.yaml", config.file);
        Assert.assertEquals(8080, config.port);
        Assert.assertTrue(config.disableDefaultExports);
